...
func WriteCollectionQueryCommand(w io.Writer, collectionName string, queryBSON []byte) error {
	...
	if err := WriteBytes(w, queryBSON); err != nil {
		return fmt.Errorf("failed to write query BSON (collection query): %w", err)
	}
	return nil
}

// ReadCollectionQueryCommand reads a QUERY_COLLECTION command from the connection.
func ReadCollectionQueryCommand(r io.Reader) (collectionName string, queryBSON []byte, err error) {
	collectionName, err = ReadString(r)
	if err != nil {
		return "", nil, fmt.Errorf("failed to read collection name (collection query): %w", err)
	}
	queryBSON, err = ReadBytes(r)
	if err != nil {
		return "", nil, fmt.Errorf("failed to read query BSON (collection query): %w", err)
	}
	return collectionName, queryBSON, nil
}
