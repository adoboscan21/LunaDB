...
func LoadCollectionData(collectionName string, s store.DataStore, hotThreshold time.Time) error {
	...
	for i := 0; i < int(numEntries); i++ {
		...
		if !hotThreshold.IsZero() {
			// Zero-copy lookup nativo con bson
			if createdAtVal, err := bson.Raw(valBytes).LookupErr(globalconst.CREATED_AT); err == nil {
				if createdAtVal.Type == bson.TypeDateTime {
					if createdAtVal.Time().Before(hotThreshold) {
						coldDataCount++
						continue
					}
				}
