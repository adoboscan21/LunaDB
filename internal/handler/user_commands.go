...
func (h *ConnectionHandler) HandleUserCreate(r io.Reader, conn net.Conn) {
	...
	username, password, permissionsBSON, err := protocol.ReadUserCreateCommand(r)
	if err != nil {
	...
	if err := bson.Unmarshal(permissionsBSON, &permissions); err != nil {
		if conn != nil {
			protocol.WriteResponse(conn, protocol.StatusBadRequest, "Invalid permissions BSON format", nil)
		}
		return
	}
...
func (h *ConnectionHandler) HandleUserUpdate(r io.Reader, conn net.Conn) {
	...
	username, permissionsBSON, err := protocol.ReadUserUpdateCommand(r)
	if err != nil {
	...
	if err := bson.Unmarshal(permissionsBSON, &newPermissions); err != nil {
		if conn != nil {
			protocol.WriteResponse(conn, protocol.StatusBadRequest, "Invalid permissions BSON format", nil)
		}
		return
	...
	bson.Unmarshal(userData, &userInfo)
