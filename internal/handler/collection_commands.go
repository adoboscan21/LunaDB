...
// processes the CmdCollectionList command. It is a read-only operation.
func (h *ConnectionHandler) handleCollectionList(r io.Reader, conn net.Conn) {
	...
	accessibleCollections := make([]string, 0)
	...
	// Envolvemos el array en un objeto BSON {"list": [...]}
	bsonData, err := bson.Marshal(bson.M{"list": accessibleCollections})
	if err != nil {
		slog.Error("Failed to marshal collection names to BSON", "error", err)
		protocol.WriteResponse(conn, protocol.StatusError, "Failed to marshal collection names", nil)
		return
	}

	if err := protocol.WriteResponse(conn, protocol.StatusOk, "OK: Accessible collections listed", bsonData); err != nil {
...
func (h *ConnectionHandler) handleCollectionIndexList(r io.Reader, conn net.Conn) {
	...
	// Envolvemos el array en un objeto BSON {"list": [...]}
	bsonResponse, err := bson.Marshal(bson.M{"list": indexedFields})
	if err != nil {
	...
	if err := protocol.WriteResponse(conn, protocol.StatusOk, fmt.Sprintf("OK: Indexes for collection '%s' retrieved.", collectionName), bsonResponse); err != nil {
